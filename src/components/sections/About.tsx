...
export default function About() {
  ...
  const tabActiveBg = theme === "dark" ? "bg-blue-600" : "bg-blue-600";
  const tabInactiveBg = theme === "dark" ? "bg-gray-700" : "bg-white";
  const tabInactiveText =
    theme === "dark"
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-12 mb-16">
          ...
          <div className="space-y-6">
            <div
              className={`${cardBg} p-8 rounded-2xl shadow-lg transition-colors duration-300`}
            >
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className={textMuted}>이름:</span>
                  <p className={`font-medium ${textPrimary}`}>전지윤</p>
                </div>
                <div>
                  <span className={textMuted}>전문분야:</span>
                  <p className={`font-medium ${textPrimary}`}>Web Design</p>
                </div>
                <div>
                  <span className={textMuted}>경력:</span>
                  <p className={`font-medium ${textPrimary}`}>13 Years</p>
                </div>
                <div>
                  <span className={textMuted}>위치:</span>
                  <p className={`font-medium ${textPrimary}`}>Suwon, Korea</p>
                </div>
              </div>
          ...
          <div>
            {/* 탭 메뉴 */}
            <div
              className={`flex space-x-1 ${cardBg} p-1 rounded-xl shadow-lg mb-6 transition-colors duration-300`}
            >
            ...
            {/* 탭 내용 */}
            <div
              className={`${cardBg} p-8 rounded-2xl shadow-lg min-h-[400px] transition-colors duration-300`}
            >
              {activeTab === "skills" && (
                      ...
                      <div className="space-y-3">
                        {skillGroup.items.map((skill, skillIndex) => (
                          <div key={skillIndex}>
                            ...
                            <div
                              className={`w-full ${progressBg} rounded-full h-2`}
                            >
                              <div
              ...
              {activeTab === "experience" && (
                <div className="space-y-6">
                  <h4 className={`text-lg font-semibold ${textPrimary} mb-6`}>
                    주요 경험
                  </h4>
                  {experiences.map((exp, index) => (
                    ...
                    >
                      <div className="flex items-center space-x-3 mb-2">
                        ...
                        <h5 className={`text-lg font-semibold ${textPrimary}`}>
                          {exp.title}
                        </h5>
                      </div>
                      <p className={textSecondary}>{exp.description}</p>
                    </div>
                  ))}
                </div>
              )}
